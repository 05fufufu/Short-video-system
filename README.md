...
docker-compose up -d
```
> ⚠️ **注意**: 首次启动可能需要几分钟下载镜像。确保 3307, 3308, 5672, 15672, 6379, 9000, 9001 端口未被占用。
...
### 切换 局域网/公网 模式
默认配置可能使用了内网穿透域名。如果你在本地或局域网演示，请参考 [BACKUP_PLAN.md](./BACKUP_PLAN.md) 进行详细配置，或按以下步骤修改：
...
4.  **公网穿透模式 (cpolar)**:
    ...
    ```
5.  **MinIO 连接配置**:
    *   找到 `MinioInnerEndpoint` 变量。
    *   如果你在本地运行 Docker，请将其修改为 `localhost:9000`。
    *   如果你在虚拟机或其他机器运行 Docker，请填入对应的 IP:9000。
6.  **重启后端服务**使配置生效。
...
### 1. 架构说明
*   **tiktok_db (主库)**: 存储视频、评论、点赞、笔记等核心业务数据。Docker 环境中包含 `mysql-slave` 从库，支持读写分离。
